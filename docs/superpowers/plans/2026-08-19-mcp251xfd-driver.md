...
> **Task 16 summary (done, commits e96f376 + 977000c + fix 5418f59):** Standalone `examples/rp2040` crate landed (own `[workspace]`, embassy-rp 0.4 stack) with `enumerate` and `loopback` binaries, plus the CI `examples` job un-gated (the `TODO(task 16)` hashFiles gate removed). Review verified the pin map against embassy-rp 0.4.0's `Spi::new` signature (SCK=GP10/MOSI=GP11/MISO=GP12, CS on GP3–9/13–15), the 17 MHz SPI cap against `max_spi_hz(40 MHz)`/the erratum, and the init→apply_layout→set_filter→set_mode ordering against the driver docs — all MATCH. **Critical caught before hardware:** the plan's literal `.cargo/config.toml` misses embassy-rp's `link-rp.x` linker fragment, leaving `.boot2` orphaned and the BOOT2 region (0x10000000) empty — a cleanly-linking, non-bootable image whose failure would masquerade as wiring problems. Fix round added `-Tlink-rp.x` + `--nmagic` (`.boot2` verified at 0x10000000 via llvm-objdump on both binaries), dropped a redundant `fixed = "=1.30.0"` pin (committed Cargo.lock already freezes it), and extended the 20 MHz-crystal note (needs `MHZ20` + hand-built timings + 8.5 MHz SPI). An `examples/rp2040/.gitignore` was added so the root gitignore's `/target` + bare-`Cargo.lock` patterns neither bulk-stage build cache nor drop the required lockfile. Zero-warning `thumbv6m-none-eabi` build confirmed on a fresh clone; Step 7 (on-hardware run) remains with the owner.
