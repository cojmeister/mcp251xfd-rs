//! The control for `bench_interference`: same sweep, **blocking** driver.
//!
//! `bench_interference` delays core 0's servicing of `DMA_IRQ_0` and the
//! MCP2517FD TX MAB underflow appears, scaling with the delay. This binary
//! applies the identical interference to the identical CAN workload with the
//! blocking driver, which uses no DMA and therefore raises no completion
//! interrupt for core 0 to be late with.
//!
//! Prediction: **zero stalls at every sweep value**. If that holds, the
//! cross-core DMA completion is the mechanism, and items 3 and 4 of the field
//! report are one defect. If this binary also faults, the mechanism is
//! something else that the interference happens to disturb, and the
//! same-defect claim fails.
//!
//! # Why this exists
//!
    ...
    loop {
        ...
        if let Ok(con) = chips[BUSSED].control_register() {
            if matches!(
                ...
                if !reported {
                    reported = true;
                    report_signature(&mut chips[BUSSED], d);
                }
                let t0 = Instant::now();
...
fn report_signature(can: &mut Can, d_us: u32) {
...
async fn sweep_task() {
    ...
    info!("bench_interference_blocking: CONTROL -- same sweep, blocking driver, no DMA");
    info!(
