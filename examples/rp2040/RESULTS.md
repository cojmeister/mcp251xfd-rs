...
Every number below came off this board. Nothing here is simulated.
...
`bench_async` runs the configuration the field report describes — async driver
on core 1, so its SPI DMA completions are raised there and serviced on core 0 —
...
instant it fired. The field rig's core 0 was busy running a flight-control
cycle. That missing variable is the entire mechanism, and it is controllable.
...
draining nothing — the exact "FIFO has space, so it must be healthy" reading
that cost the reporting project 700 false positives, captured mid-fault.
...
**Conclusion: the cross-core DMA completion is the mechanism.** Items 3 and 4 of
the field report are one defect, and moving the CAN workload to the blocking
...
For comparison, the recovery ladder the reporting project had been using — a
full Configuration-mode cycle re-applying layout and filters — measured
**22,000 µs**, eleven times their 2 ms budget, which is why they kept it out of
production. A median of 137–246 µs fits inside that budget with room to spare.
...
| `bench_async` | CAN bus | Async driver on core 1 — the field-faulting configuration |
